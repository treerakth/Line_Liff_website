...
export default function DetailCoupon() {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (err.message == "Coupon already collected") {
        showSuccessAlert("สำเร็จ", "ท่านได้ลงทะเบียน คูปองแล้ว", "/my-coupons");
      } else {
        showErrorAlert("เกิดข้อผิดพลาด", err.message, "/my-coupons");
      }
    } finally {
      ...
      <div className="w-full max-w-md flex-1">
        ...
        <section className="space-y-4 mb-6 text-base md:text-lg">
          ...
          <div>
            ...
            <p>
              {coupon.description}{" "}
              <span className="text-red-600 font-bold">{coupon.term}</span>
            </p>
          </div>
          <div>
